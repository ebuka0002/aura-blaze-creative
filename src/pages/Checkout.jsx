...
import { NIGERIA_STATES, getNigeriaCities } from '../data/nigeriaLocations'
...
export default function Checkout() {
  ...
  const handleStateChange = (e) => {
    ...
    setForm((prev) => ({ ...prev, state, city: '', postalCode: '' }))
    setShippingRates([])
  ...
  <>
    ...
    >
      ...
      {NIGERIA_STATES.map((state) => (
        <option key={state} value={state}>{state === 'Abuja' ? 'Abuja (FCT)' : state}</option>
      ))}
    </select>
    ...
    <select
      ...
      disabled={!form.state}
      className="border border-hairline px-4 py-3.5 text-sm focus:outline-none focus:border-blaze bg-bone disabled:opacity-60"
    >
      <option value="" disabled>{form.state ? 'Select city / area' : 'Select state first'}</option>
      {getNigeriaCities(form.state).map((city) => (
